...
BB <- read_csv("/Users/troymcdiarmid/Desktop/BB_Filtered_Edit_Scores_Comparison_Table.csv") 
...
##Finding how many within five-fold of standard accross both BC and cell contexts 
...
MW_BC_Pool1_Within_5x_Standard <- MW_BC_Pool1 %>% 
  filter(K562 > (0.2*(median(Standard_BC_Pool_1$K562)))) %>% 
  filter(HEK293T > (0.2*(median(Standard_BC_Pool_1$HEK293T)))) %>% 
  filter(iPSC > (0.2*(median(Standard_BC_Pool_1$iPSC))))
...
MW_BC_Pool2_Within_5x_Standard <- MW_BC_Pool2 %>% 
  filter(K562 > (0.2*(median(Standard_BC_Pool_2$K562)))) %>% 
  filter(HEK293T > (0.2*(median(Standard_BC_Pool_2$HEK293T)))) %>% 
  filter(iPSC > (0.2*(median(Standard_BC_Pool_2$iPSC))))

##Within Within 5x of median of standard across barcodes and contexts

Within_5x_Standard <- MW_BC_Pool1_Within_5x_Standard %>% 
  filter(ID_Number %in% MW_BC_Pool2_Within_5x_Standard$ID_Number) %>% 
  filter(!Variant_Type == "Standard") %>% 
  filter(!Variant_Type == "R:AR")

ggplot(Within_5x_Standard, aes(x = Variant_Position, y = K562)) +
  geom_point() +
  xlim(0,235) +
  ylim(0,max(Within_5x_Standard$K562+1))

##Finding how many within within 50x of standard across both BC and cell contexts 
...
MW_BC_Pool1_Within_50x_Standard <- MW_BC_Pool1 %>% 
  filter(K562 > (0.02*(median(Standard_BC_Pool_1$K562)))) %>% 
  filter(HEK293T > (0.02*(median(Standard_BC_Pool_1$HEK293T)))) %>% 
  filter(iPSC > (0.02*(median(Standard_BC_Pool_1$iPSC))))
...
MW_BC_Pool2_Within_50x_Standard <- MW_BC_Pool2 %>% 
  filter(K562 > (0.02*(median(Standard_BC_Pool_2$K562)))) %>% 
  filter(HEK293T > (0.02*(median(Standard_BC_Pool_2$HEK293T)))) %>% 
  filter(iPSC > (0.02*(median(Standard_BC_Pool_2$iPSC))))

##Within 5x of median of standard across barcodes and contexts

Within_50x_Standard <- MW_BC_Pool1_Within_50x_Standard %>% 
  filter(ID_Number %in% MW_BC_Pool2_Within_50x_Standard$ID_Number) %>% 
  filter(!Variant_Type == "Standard") %>% 
  filter(!Variant_Type == "R:AR")

ggplot(Within_50x_Standard, aes(x = Variant_Position, y = K562)) +
  geom_point() +
  xlim(0,235) +
  ylim(0,max(Within_50x_Standard$K562+1))


##Rewriting table

##Adding within 50x standard column

MW_Within_50x_Standard <- MW %>% 
  filter(ID_Number %in% Within_50x_Standard$ID_Number)

MW_Within_50x_Standard$Within_50x_Standard <- "TRUE"

MW_Other <- MW %>% 
  filter(!ID_Number %in% Within_50x_Standard$ID_Number)

MW_Other$Within_50x_Standard <- "FALSE"

MW <- rbind(MW_Within_50x_Standard, MW_Other)

##Adding within 5x standard column

MW_Within_5x_Standard <- MW %>% 
  filter(ID_Number %in% Within_5x_Standard$ID_Number)

MW_Within_5x_Standard$Within_5x_Standard <- "TRUE"

MW_Other <- MW %>% 
  filter(!ID_Number %in% Within_5x_Standard$ID_Number)

MW_Other$Within_5x_Standard <- "FALSE"

MW <- rbind(MW_Within_5x_Standard, MW_Other)
