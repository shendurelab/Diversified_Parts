...
Pol_III_Pro_Edit_Scores <- read_csv("/Users/troymcdiarmid/Documents/U6_pro_series/Figs/Pub_Figs/FigS17_Final_Figure_Datasets/Pol_III_Pro_Edit_Scores.csv")
...
H1_Del <- H1 %>% 
  filter(SNV_Change == "Del")

ggplot(H1_Del, aes(x = Variant_Position, y = Log2_Fold_Change_Relative_To_Standard)) +
  geom_line() +
  xlim(1,100) +
  theme_classic() +
  theme_minimal()

H1_Not_Del_Average <- H1 %>% 
  filter(!SNV_Change == "Del") %>% 
  group_by(Variant_Position) %>% 
  mutate(Mean_Log2_Fold_Change_Relative_To_Standard = mean(Log2_Fold_Change_Relative_To_Standard)) %>%
  ungroup()

ggplot(H1_Not_Del_Average, aes(x = Variant_Position, y = Log2_Fold_Change_Relative_To_Standard)) +
  geom_line() +
  xlim(1,100) +
  theme_classic() +
  theme_minimal()

##Now doing it for 7SK

SevenSK_Del <- SevenSK %>% 
  filter(SNV_Change == "Del")

ggplot(SevenSK_Del, aes(x = Variant_Position, y = Log2_Fold_Change_Relative_To_Standard)) +
  geom_line() +
  xlim(1,243) +
  theme_classic() +
  theme_minimal()

SevenSK_Not_Del_Average <- SevenSK %>% 
  filter(!SNV_Change == "Del") %>% 
  group_by(Variant_Position) %>% 
  mutate(Mean_Log2_Fold_Change_Relative_To_Standard = mean(Log2_Fold_Change_Relative_To_Standard)) %>%
  ungroup()

ggplot(SevenSK_Not_Del_Average, aes(x = Variant_Position, y = Log2_Fold_Change_Relative_To_Standard)) +
  geom_line() +
  xlim(1,243) +
  theme_classic() +
  theme_minimal()
